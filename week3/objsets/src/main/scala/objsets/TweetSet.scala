...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    ...
    def loop(list: TweetList, tweets: TweetSet): TweetList = {
      tweets match {
        case _: Empty => list
        case e: NonEmpty =>
          val most = e.mostRetweeted
          loop(new Cons(most, list), e.remove(most))
      }
    }
    ...
    val ascending = loop(Nil, this)
    reverseList(ascending, Nil)
  }
...
class NonEmpty(val elem: Tweet, val left: TweetSet, val right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
    ...
    def loop(set: TweetSet, that: TweetSet): TweetSet = {
      set match {
        case _: Empty => that
        case e: NonEmpty =>
          val afterLeft = loop(e.left, that.incl(e.elem))
          loop(e.right, afterLeft)
      }
    }
  ...
  def mostRetweeted: Tweet = {
    ...
    def loop(set: TweetSet, tweet: Tweet): Tweet = {
      set match {
        case _: Empty => tweet
        case e: NonEmpty =>
          loop(e.left, if (e.elem.retweets > tweet.retweets) e.elem else tweet)
          loop(e.right, if (e.elem.retweets > tweet.retweets) e.elem else tweet)
      }
    }
...
object Main extends App {
  ...
  val result = s3.filter(tweet => tweet.retweets > 1)
  println("------------------------------------------------------------------")
  println("result:")
  ...
  println("------------------------------------------------------------------")
  println("Most retweeted in all tweets: " + allTweets.mostRetweeted)
  println("------------------------------------------------------------------")
  println("Most retweeted in all tweets: ")
